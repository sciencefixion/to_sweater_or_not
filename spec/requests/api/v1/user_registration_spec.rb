...
RSpec.describe 'User Registration request' do
  it 'allows a user to create an account' do
    ...
    post '/api/v1/users', params: body.to_json, headers: { 'CONTENT_TYPE' => 'application/json', 'Accept' => 'application/json' }
    ...
    expect(registration_response[:data][:attributes][:email]).to eq(body[:email])
